...
def get_current_weather(city: str) -> dict:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.error(
                f"API Error: Code {response.status_code} for city {city}")
            return {
                "success": False,
                "error": "Ошибка связи с сервером погоды."
            }
    except requests.exceptions.RequestException as e:
        logger.error(f"Request exception: {e}")
        return {
            "success": False,
            "error": "Превышено время ожидания или ошибка сети."
        }
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for item in data["list"]:
                ...
                daily_data[date_str]["descs"].append(
            ...
            for date_str in target_dates:
                ...
                most_common_desc = max(set(descs),
                                       key=descs.count).capitalize()
            ...
            logger.error(
                f"API Error (Forecast): Code {response.status_code} for city {city}"
            )
            return {
                "success": False,
                "error": "Ошибка связи с сервером погоды."
